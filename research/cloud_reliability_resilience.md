...
+ X-Trace from Berkeley outputs a set of task graphs according to Lamport’s happens before relation. Thus, since does not rely on clocks, it is decoupled from the traditional log generated. The solution places emphasis on task graphs and overlooks the more important notion of service and process. Furthermore, the primitives used for instrumentation are still too close to programming  (e.g., xtr::logEvent(string) and xtr::logEvent("end")) and too far away from providing a service and process view to the system.
Focuses on recovering the task trees associated with multi-layer protocols, rather than the analysis of those recovered paths.
+ Magpie from Microsoft infers traces by combining event logs generated by low level black-box instrumentation. Thus, the traces generated provide a software perspective and overlook the notion of service and process.
Magpie relies on experts with deep knowledge about the system to construct a schema of how to correlate events in different components.
+ Dapper from Google also focuses on library and middleware modifications providing a close view of software dependencies and tracing.
